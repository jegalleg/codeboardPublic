...
angular.module('codeboardApp')
  .controller('ProjectStatsCtrl', ['$scope', '$log', '$route', '$http', 'StatsSrv',
    function ($scope, $log, $route, $http, StatsSrv) {
      ...
      $scope.isLoadingUserAccessGraphDataByUser = true;
       ...
       * Requires: $scope.projectAccessPerDay has the data as {_id, count}
       */
      var drawProjectAccessGraphByUser = function () {

        $scope.dataGraphUserAccessByUser = {
          labels: [],
          series: [],
          data: []
        };
        ...
        // sort the array and add the missing dates (with value 0)
        StatsSrv.sortAndAddMissingDates($scope.fromDate, $scope.untilDate, $scope.projectAccessPerDayByUser);

        $scope.dataGraphUserAccessByUser.series.push('Project accesses');
        addLineToGraph($scope.projectAccessPerDayByUser, $scope.dataGraphUserAccessByUser);
        addLabelsToGraph($scope.projectAccessPerDayByUser, $scope.dataGraphUserAccessByUser);
      };
      ...
      var getCompilationRunDataForGraphByUser = function () {
        ...
        $http.get('/api/log/user/summaryProjectCompilationRunDayByUser/' + projectId, {
          params: {
            user: $scope.usuario,
            startDateLogs: $scope.fromDate,
            endDateLogs: $scope.untilDate
          ...
          .success(function (aReply) {
            ...
            $scope.totalCompilationsDayByUser = '(' + getTotal($scope.compilerSummaryLogsByUser, 'count') + ')';
      ...
      var getProjectAccessDataForGraphByUser = function () {
        ...
        $http.get('/api/log/user/summaryProjectAccessDayByUser/' + projectId, {
          params: {
            user: $scope.usuario,
            startDateLogs: $scope.fromDate,
            endDateLogs: $scope.untilDate
          ...
          .success(function (aReply) {
            $scope.projectAccessPerDayByUser = aReply.projectAccessPerDay;
            $scope.totalAccessPerDayByUser = '(' + getTotal($scope.projectAccessPerDayByUser, 'count') + ')';
            drawProjectAccessGraphByUser();
            $scope.isLoadingUserAccessGraphDataByUser = false;
      ...
      $scope.usuario = '';
      update();
      $scope.update = function () {//hide button
        $scope.usuarioVisible = false;
        /*if ($scope.usuario != '') {
