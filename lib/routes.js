...
module.exports = function(app) {
 ...
 /**ESUS*/
  app.get('/api/users/:username/stats', mw.stats); // display data for specific user
  /** */
  ...
  //JESUS
  //app.get('/api/projects/:user/authorizedownercheck', mw.stats);
  ...
  //JESUS
  app.get('/api/log/user/summaryProjectAccess2/', mw.isAuth, logCtrl.getProjectAccessPerProjectLogs);
  ...
  app.get('/api/log/user/summaryCompilationRunDay/', mw.isAuth, logCtrl.getCompilationPerDayLogs); // get the summary of compilation and run per day for a project id
//check if the user is owner of the results in ddbb
  //**JESUS */
//  app.get('/api/log/user/getCompilationRunDataForGraphByUser/', mw.isAuth, logCtrl.getCompilationPerUserAllProjectsLogs);
  app.get('/api/log/user/getCompilationRunDataForGraphByUser/', mw.isAuth, logCtrl.getCompilationPerUserAllProjectsLogs);
  app.get('/api/log/user/getCompilationRunDataPerProjectForGraphByUser/', mw.isAuth, logCtrl.getCompilationPerUserPerProjectsLogs);
  app.get('/api/log/user/getCompilationRunDataDetailPerProjectForGraph/', mw.isAuth, logCtrl.getCompilationDetailPerUserPerProjectsLogs);//for compilation errors per project
  app.get('/api/log/user/getCompilationRunDataDetailPerTimeForGraph/', mw.isAuth, logCtrl.getCompilationDetailPerUserPerTimeLogs);//for compilation errors per time


  app.get('/api/log/user/summaryProjectCompilationRunDayByUser/:projectId', mw.isValidProjectId, mw.isAuth, mw.isOwner, logCtrl.getCompilationPerDayLogsByUser); // get the summary of compilation and run per day for a project id
  /** */
  ...
  //JESUS
  app.get('/api/log/user/summaryProjectAccessDayByUser/', mw.isAuth, logCtrl.getProjecAccessPerUserAllProjectsLogs); // get the summary of user access per day for a project id 
  app.get('/api/log/user/summaryProjectAccessDayByUser/:projectId', mw.isAuth, logCtrl.getProjecAccessPerUserAllProjectsLogs); // get the summary of user access per day for a project id 

  //JESUS route to get access data for all projects
  app.get('/api/log/user/summaryProjectAccessDayPerProject/', mw.isAuth, logCtrl.getProjectAccessLogsPerProject);  


  app.get('/api/log/user/summaryUserCompilationRunPerProject/:user',mw.isAuth,logCtrl.getUserCompilationPerProject);//only check if the user in auth
  ...
  //ruta prueba JESUS
  app.post('/api/:user/test',mw.test);
  //app.get('/api/log/user/getProjectNames/',mw.isAuth,projectCtrl.getAllProjects);
};
