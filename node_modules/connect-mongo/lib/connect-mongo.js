...
module.exports = function(connect) {
  ...
  // This is a modification of the original set function (see above).
  // We only want to store  session in Mongo that belongs to a user who authenticated via passport.
  // This modified code achieves this.
  // It's taken from here:  https://github.com/kcbanner/connect-mongo/issues/110
  // The acutal code fiddle is here: http://jsfiddle.net/YPCzU
  MongoStore.prototype.set = function(sid, session, callback) {
    try {
      var s = {_id: sid, session: this._serialize_session(session)};
      if (session && session.cookie && session.passport.user) { // custom, we need to check for session.passport.user, so ghost users will not be stored
        ...
        this._get_collection(function(collection) {
          collection.remove({_id: sid}, function(err, data) {
            if (err) {
              callback && callback(err);
            } else {
              callback && callback(null);
